...
export function readRecord(
  ...
  // Pure hack just to fix some files
  // This actually works for most files, assuming values are the same
  if (messageSize > 100) {
    // try to found the next point
    // The "in between" points are a counter
    let max = messageSize - 5;
    for (let i = 5; i < max; i++) {
      if (
        blob[startIndex + i] === 0xff &&
        blob[startIndex + i + 1] === 0x07 &&
        blob[startIndex + i + 4] === 0x3d &&
        blob[startIndex + i + 5] === 0x3a
      ) {
        messageSize = i;
        console.error('Changing message size:', messageSize, '- was:', max);
        break;
      }
    }
  }
